   ...
   //fileHandler.setLevel(Level.INFO); //Log的層級
     ...
     <%     request.setCharacterEncoding("UTF-8");
	    ...
	    if( request.getParameter("fix") != null ){ 
              ...
              log.info("insert id="+c_id2[i] +" name="+name2[i]+" code="+code2[i]+" youad="+stri[i]);
               String sql = "update send set youad = '" + stri[i] + "',name = '" + name2[i] + "',code = '" + code2[i] + "',youad2 = '" +  yad +"',memo = '" + memo[i]+"'  where c_id ='" + c_id2[i] + "' "; 
	...
	%> 
        ...
        <%  if(request.getParameter("update") != null && a!=null)
           { 
             for(i=0;i<a.length;i++){
                String sql="select * from send where c_id = '" + a[i] + "'order by c_id ASC ";
     ...
     客戶代號：  <input type = "text" name= "c_id" size = "6" value = "<%=a[i]%>"> 
      ...
      客戶地址：  <input type = "text" name= "str" size = "64" value = "<%=str%>"> </br></br>
      備註：  <input type = "text" name= "memo" size = "64" value = "<%=rs.getString("memo")%>"> 
       </br></br>
	 <% } if(i==a.length-1) { %>
...
<%}}}%> 
  ...
  <br>
    ...
    <thead>
    <b><tr><th>選項</th><th>客戶代號</th><th>客戶名稱</th><th>郵遞區號</th><th>客戶地址</th><th>備註</th></tr></b>
    </thead>
    <tbody id="myTable">
        <%     
	 ...
	 out.print("<tr><td><center>"+"<input type=checkbox name='a' value="+c_id+">"+"</center></td><td>"+ c_id + "</td><td>"+name+"</td><td> "+ code+"</td><td> " + youad+ youad2+"</td><td> "+ memo2+ "</td></tr> "); 
                   }
	                rs.close(); 
	        %>
